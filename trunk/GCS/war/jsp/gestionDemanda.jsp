							...
							<c:otherwise>
								<c:forEach items="${demandaList}" var="demanda">
									<tr class="valid-result" id="row${demanda.key.id}" data-fecha-comun="${demanda.str_fecha_comunicacion}" data-hora-comun="${demanda.hora_comunicacion}" data-gestor-asig="${demanda.gestor_it}">
										...
										<c:if test="${sessionScope.permiso != 5 and sessionScope.permiso != 4}">
											<td><img class="vs" src="../img/vs.png"><a class="lapiz" data-target="#edit-demanda" data-toggle="modal" name="${demanda.key.id}"	id="lapiz${demanda.key.id}"></a><a class="papelera" name="${demanda.key.id}" data-toggle="modal" data-target="#confirm-delete" id="papelera${demanda.key.id}"></a></td>
										</c:if>
									</tr>
								</c:forEach>
							</c:otherwise>
	...
	<div class="modal fade" id="edit-demanda" tabindex="-1" role="dialog"
		...
		<div class="modal-dialog">
					...
					<div class="form-field-divider left">
						...
						<div class="form-field">
							...
							<div class="input">
								...
								<select class="selectpicker selected time" id="hora_peticion_modal" name="hora_peticion" required aria-required="true">
									<c:forEach items="${horasList}" var="hora">
									<option value="${hora}">${hora}</option>
									</c:forEach>
								</select>
								<span class="time-span">:</span>
								<select class="selectpicker selected time" id="min_peticion_modal" name="min_peticion" required aria-required="true">
									<c:forEach items="${minutosList}" var="min">
									<option value="${min}">${min}</option>
									</c:forEach>
								</select>
			...
			<div class="modal-footer">
				<button id="edit_demanda_form_modal" class="" type="button">Guardar</button>
				<button data-dismiss="modal" class="" type="button">Cancelar</button>
			</div>		
		</div>
	</div>
</div>
