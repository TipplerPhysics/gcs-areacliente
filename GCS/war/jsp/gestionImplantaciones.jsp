...
<div class="gestion_implantacion">
	...
	<div class="breadcrumbs">
		<span onclick="window.location.href='../../' ">Home</span> > <span>
			Registro de implantaciones </span>
	</div>

	<div class="newUserbox">

		<c:if
			test="${sessionScope.permiso != 5 and sessionScope.permiso != 4}">
			<button id="sendMailButton"
				<c:if test="${puedeGenerarInforme == 'true'}"> disabled="true"</c:if>
				<c:if test="${implantacionEnCurso == 'false'}"> disabled="true"</c:if>>
				Enviar correo Negocio<span class="demanda_span"></span>
			</button>
			<button id="generarInformeButton"
				<c:if test="${puedeGenerarInforme == 'false'}"> disabled="true"</c:if>>
				Generar informe<span class="demanda_span"></span>
			</button>
		</c:if>
		<button id="excel_btn"
			onclick="window.location.href='../../implantacionServlet?accion=xls'">
			...
			<div class="table-responsive usersTable">
				<table class="table">
					<thead>
						...
						<tr>
							<th><label for="check_all_implantaciones">&nbsp;</label> <input
								type="checkbox" name="check_all_implantaciones"
								id="check_all_implantaciones"
								<c:if test="${implantacionEnCurso == 'true'}"> checked="true" disabled="true"</c:if> />
							</th>
							<th class="search-th"><input
								class="search col0 search_anywhere"></th>
						...
						<c:choose>
							...
							<c:otherwise>
								<c:forEach items="${implantacionList}" var="implantacion"
									varStatus="loop">
									<tr class="valid-result" id="row${loop.index}"
										data-servicio-id="${implantacion.serviciokey}"
										data-conectividad-id="${implantacion.conectividadkey}">
										<td><label for="check${loop.index}">&nbsp;</label> <input
											type="checkbox" class="inner" name="check${loop.index}"
											value="" id="check${loop.index}"
											<c:if test="${implantacionEnCurso == true}"> checked="true" disabled="true"</c:if> />
										</td>
										...
										<c:if
											test="${sessionScope.permiso != 5 and sessionScope.permiso != 4}">
											<td><c:forEach items="${conectividades}"
													var="conectividad">
													<c:if
														...
														<form id='updateimp${conectividad.key.id}'>
															<div class="controls">
																<div class='form-field'>
																	<span>Estado subida:</span> <select id="estadoSubid"
																		class="selectpicker selected" name="estadoSubid">
																		<option value="OK"
																			${conectividad.estadoSubida == 'OK' ? 'selected' : ''}>OK</option>
																		<option value="KO"
																			${conectividad.estadoSubida == 'KO' ? 'selected' : ''}>KO</option>
																	</select>
																</div>

																<div class='form-field'>
																	<span class="top"> Detalles:</span>
																	<textarea placeholder='Introduzca texto'
																		name="detalleSubid">${conectividad.detalleSubida}</textarea>
																</div>
															</div>
															<div class="buttons">
																<button type='button' data-id='${conectividad.key.id}'
																	data-tipo='conectividad' class="subidaModifImp">
																	Aceptar</button>
																<button type='button' class="dropbutton"
																	data-id=${conectividad.key.id}>Cancelar</button>
															</div>
														</form>
													</td>
												</tr>
												...
												<tr id="line${servicio.key.id}" class="modifHolder oculto">
													<td class="edit-impl-row" colspan="8">
														...
														<div class='form-field'>
															...
															<div class="buttons">
																<button type='button' data-id='${servicio.key.id}'
																	data-tipo='servicio' class="subidaModifImp">
																	Aceptar</button>
																<button type='button' class=" dropbutton"
																	data-id=${servicio.key.id}>Cancelar</button>
															</div>
														</form>
													</td>
												</tr>
	...
	<div class="modal fade" id="send-email-implantacion" tabindex="-1"
		role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
		<div class="modal-dialog" id="send_email_dialog">
			<div class="modal-content"></div>
		</div>
	</div>
	<div class="modal fade" id="redirect-informe" tabindex="-1"
		role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
		<div class="modal-dialog" id="send_email_dialog">
			<div class="modal-content"></div>
		</div>
