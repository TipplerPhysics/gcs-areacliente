...
/*
 * Codigo JavaScript para controlar todas posibles opciones del mantenimiento de servicio
 */
...
function sendEditCatalogoService(){
	
	
	var nombreServicio = $('#name').val();
	var nombrePais = $('#pais').val();
	var nombreExtension = $('#extensiones').val();

	
	var $form = $('#edit-CatalagoServicio-form');
	var formURL = $form.attr("action");
	var $formData = $form.serialize();
	var postData= $formData+"&accion=update&id="+ id;
		 ...
		 $.ajax(			
			{
				...
				success:function(data, textStatus, jqXHR) 
				{
					if (data.success="true"){
						$form.hide();
						$('#span_message_modal').html('El pa&iacute, el servicio y extensiones han sido registrado de forma correcta.<br/>En breve volvemos a la p&aacute;gina.');
						$('.modal-footer').hide();
						$('#message_div_modal').css('display','block').removeClass("error").addClass("success");
...
function editCatalogoService(id){	
	showModal();
}

/*
 * Codigo JavaScrpit para controlar todas las posibles acciones de la pantalla de mantenimiento de Catalogo de Servicios
 */

$(function(){
	
	$('#edit-catalogoServicio').on('loaded.bs.modal', function () {
		editCatalogoService(id);
	})
	
	$('.gestion_catServicio').on('click', '.lapiz', function(e) {
		id= $(this).attr('name');		
	});
	
	$('.gestion_catServicio').on('click', '.papelera', function(e) {
		$('#deleteCatalogo').attr('name',$(this).attr('name'));
	});
	
	$('#deleteCatalogo').on('click', function(e) {
		var id= $(this).attr('name');
		 var formURL = "/catalogoServlet?";
		 var postData="accion=delete&id="+ id;
		 $.ajax(			
			{
				url : formURL,
	...
	$('#new-catalgoService-form').on('change', '#pais_servicio', function(e) {
		var pais = $('#pais_servicio').val();
		var target = $('#servicio');
		var targetExt = $('#extension');
		ajaxServicios(pais,target,targetExt);		
	...
	$('#new-catalgoService-form').on('change', '#servicio', function(e) {
		var pais = $('#pais_servicio').val();
		var servicio = $('#servicio').val();
		var target = $('#extension');
		ajaxExtensiones(servicio,pais,target);
	...
	$('#new-catalgoService-form').on('change', '#id_cliente', function(e) {
		var cliente = $('#id_cliente').val();
		var target = $('#cod_proyecto');
		ajaxProyectos(cliente,target);
...
*/	$('html').on('change', '#pais_servicio_modal', function(e) {
	...
	$('body').on('click', '#submit_catalogService_form', function(e) {
		e.preventDefault(); //STOP default action
		var $form = $("#new-catalgoService-form");
		
		if($form.valid()){		
			$('#new-catalgoService-form').addClass('hidden');
			var postData = $form.serialize() + "&accion=new";
			var formURL = $form.attr("action");
			$.ajax(
					{
						  ...
						  success:function(data, textStatus, jqXHR) 
						  {
							...
							if (data.success==("true")){
								...
								$form.find('.form-container').find('div:not(#message_div)').hide(0);
								$form.find('#span_message').html('El pa&iacutes, servicio y extensiones han sido creado de forma correcta.<br/>En breve volvemos a la p&aacute;gina.');
